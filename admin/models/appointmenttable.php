...
class EventtableeditModelAppointmenttable extends JModelAdmin
{
    /**
     ...
     * @since	1.6
     */
    public function getTable($type = 'Etetable', $prefix = 'EventtableeditTable', $config = [])
    {
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_eventtableedit.appointmenttable', 'appointmenttable', ['control' => 'jform', 'load_data' => $loadData]);
        if (empty($form)) {
            return false;
        }
    ...
    public function loadFormData()
    {
        ...
        $data = JFactory::getApplication()->getUserState('com_eventtableedit.edit.etetable.data', []);
    ...
    public function save($data)
    {
        ...
        $result = $dispatcher->trigger($this->event_before_save, [$this->option.'.'.$this->name, $table, $isNew]);
        ...
        $dispatcher->trigger($this->event_after_save, [$this->option.'.'.$this->name, $table, $isNew]);
        ...
        if (0 === (int)$data['id'] && 1 === (int)$data['temps']) {
            ...
            if ($data['col'] > 0) {
                for ($i = 1; $i <= $data['col']; ++$i) {
                    ...
                    $ins = sprintf("INSERT INTO #__eventtableedit_heads (`id`,`table_id`,`name`,`datatype`,`ordering`) VALUES ('',%s,'%s','text',%s)", $table->id, $nameofhead, $i);
                    $db->setQuery($ins);
                    $db->query();
                    $newId = $db->insertid();
                    $this->updateRowsTable('0', $newId, $table->id);
            ...
            if (count($data['headdata']['linehead']) > 0) {
                for ($i = 0; $i <= count($data['headdata']['linehead']) - 1; ++$i) {
                    ...
                    $ins = sprintf("INSERT INTO #__eventtableedit_heads (`id`,`table_id`,`name`,`datatype`,`ordering`) VALUES ('',%s,'%s','%s',%s)", $table->id, $nameofhead, $datatype, $i);
                    $db->setQuery($ins);
                    $db->query();
                    $newId = $db->insertid();
